...
mythFFmpegEncoder* mythFFmpegEncoder::CreateNew(void* phwnd, int streamtype, int level,int width,int height)
{
	return new mythFFmpegEncoder(phwnd, streamtype, level,width,height);
}
bool mythFFmpegEncoder::Init(){
	...
	case 0:
		...
		break;
	case 1:
		video_codec = avcodec_find_encoder(AV_CODEC_ID_H265);
		break;
	default:
		video_codec = avcodec_find_encoder(AV_CODEC_ID_H264);
		break;
	...
	case 8:
		...
		break;
	case 9:
		av_opt_set(c->priv_data, "preset", "placebo", 0);
		break;
	default:
		av_opt_set(c->priv_data, "preset", "ultrafast", 0);
		break;
